...
class BrowserPoolManager:
    ...
    def __init__(
        ...
        self.browser_args: BrowserArgs = browser_args or {}
        self.default_config: RenderConfig = default_config or RenderConfig()
        
        # 浏览器池状态
        self._playwright: Playwright | None = None
        self._browser_pool: dict[BrowserType, list[Browser]] = {}
        self._available_browsers: dict[BrowserType, list[Browser]] = {}
        self._page_pool: dict[Browser, list[Page]] = {}
        self._available_pages: dict[Browser, list[Page]] = {}
    ...
    async def _try_acquire_page(self, browser_type: BrowserType) -> tuple[Browser, Page]:
        ...
        async with self._lock:
            # 检查可用页面
            if self._available_pages.get(browser):
    ...
    async def _acquire_browser(self, browser_type: BrowserType) -> Browser:
        ...
        async with self._lock:
            ...
            if len(self._browser_pool.get(browser_type, [])) < self.max_browsers:
                ...
                self._browser_pool.setdefault(browser_type, []).append(browser)
                return browser
    ...
    async def _create_browser(self, browser_type: BrowserType) -> Browser:
        ...
        browser = await browser_creator.launch(**launch_args)
        
        # 初始化池
        self._page_pool[browser] = []
