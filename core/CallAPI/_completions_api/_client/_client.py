...
class ClientBase(ABC):
    ...
    async def _print_log(self, user_id: str, request: Request, response: Response):
        ...
        logger.info(f"Total Time: {total_time / 10**9:.2f}s({format_time_duration(total_time, use_abbreviation=True)})", user_id = user_id)
        requests_time = response.calling_log.request_end_time.monotonic - response.calling_log.request_start_time.monotonic
        logger.info(f"API Request Time: {requests_time / 10**9:.2f}s({format_time_duration(requests_time, use_abbreviation=True)})", user_id = user_id)
        ...
        logger.info(f"Stream Processing Time: {stream_processing_time / 10**9:.2f}s({format_time_duration(stream_processing_time, use_abbreviation=True)})", user_id = user_id)
        ...
        if response.calling_log.total_chunk > 0:
            ...
            logger.info(f"Chunk Average Spawn Time: {ave_chunk_spawn_time / 10**6:.2f}ms({format_time_duration(ave_chunk_spawn_time, use_abbreviation=True)})", user_id = user_id)
            logger.info(f"Chunk Max Spawn Time: {max_chunk_spawn_time / 10**6:.2f}ms({format_time_duration(max_chunk_spawn_time, use_abbreviation=True)})", user_id = user_id)
            logger.info(f"Chunk Min Spawn Time: {min_chunk_spawn_time / 10**6:.2f}ms({format_time_duration(min_chunk_spawn_time, use_abbreviation=True)})", user_id = user_id)
